...
const TodosAlunos = (props) => {
    ...
    const [aluntos, setAlunos] = useState([]);
    ...
    const carregarRegistro = () => {
        ...
        fetch(`${process.env.REACT_APP_DOMAIN_API}/api/aluno/`, {
            headers: {
    ...
    function pesquisar() {
        ...
        fetch(
            `${process.env.REACT_APP_DOMAIN_API}/api/aluno/search?pesquisa=${pesquisa}`,
            params
        ).then((response) => {
    ...
    return (
      ...
      <hr></hr> */}
                ...
                <TextField
                    ...
                    id="filled-basic"
                    // variant="filled"
                    label="Informe o nome ou cpf do aluno"
                    name="pesquisa"
                    value={pesquisa}
        ...
        <button style={{ padding: '8px 16px', margin: '0 5px', backgroundColor: '#007bff', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', transition: 'background-color 0.3s ease' }} onClick={() => setCurrentPage(prev => prev + 1)}>Próxima</button>
                ...
                <div>
                    {turmaSelecionada.map((item, index) => (
                        <div>
                            {turmaSelecionada
                                .sort((a, b) => {
                                    // Prioriza os itens onde alunoVinculado ou emailCriado são false
                                    const aPriority = !a.alunoVinculado || !a.emailCriado ? 0 : 1;
                                    const bPriority = !b.alunoVinculado || !b.emailCriado ? 0 : 1;
                                ...
                                .map((item, index) => (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            color: '#333',
                                            padding: '16px',
                                            backgroundColor: item.alunoVinculado === false || item.emailCriado === false ? '#FFCCCC' : '#fff',
                                            borderRadius: '8px',
                                            marginBottom: '8px',
                                    ...
                                    >
                                        ...
                                        <div style={{ fontSize: 15, marginLeft: 8, marginRight: 8, position: 'relative' }}>
                                            <b>Emails Senac Criado:</b> {item.emailCriado ? 'sim' : 'não'}
                                        </div>
                                        <div style={{ fontSize: 15, marginLeft: 8, marginRight: 8, position: 'relative' }}>
                                            <b>Email Senac:</b> {item.email}
                                        </div>
                                        <div style={{ fontSize: 15, marginLeft: 8, marginRight: 8, position: 'relative' }}>
                                            <b>Vinculado:</b> {item.alunoVinculado ? 'sim' : 'não'}
                                        </div>
                                    </div>
                                ))}
            ...
            {aluntos.length > 0 && turmaSelecionada.length === 0 ? (
                <div>
                    {aluntos
                        .sort((a, b) => {
                            // Prioriza os itens onde alunoVinculado ou emailCriado são false
